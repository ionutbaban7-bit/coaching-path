...
try{
  ...
  for(const name of ['descopera','competente','povesti','intrebari','greseli','invata','resurse','certificari','hub','forum']){
    const r=await fetch(base+'/'+name+'/');check(r.status===200,name+': clean route');check((await r.text()).includes('academy.css?v=4.0.0'),name+': current assets');check(r.headers.get('content-security-policy').includes("default-src 'self'"),'CSP preserved');
  }
  ...
  r=await fetch(base+'/assets/js/academy.js?v=4.0.0',{method:'HEAD'});check(r.status===200&&r.headers.get('cache-control').includes('immutable'),'versioned script caching');
